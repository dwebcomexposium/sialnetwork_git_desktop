...
/*!
 * Comexposium Exhibitor Search Form Module
 *
 * Redesign by Alsacréations (alsacreations.fr)
 * Code design contributors: Geoffrey Crofte (Alsacréations)
 * Vendors included : empty
 *
 * File Last Update: 2014-08-26
 */
...
;jQuery(document).ready(function($){
 	...
 	if ( $('.exhibitor-search-form').length === 1 ) {
		...
		var esf={log:function(e){"undefined"!=typeof console&&console.log(e)}},
			esf_text = {
				remove_filter : 'Retirer ce filtre',
 		...
 		*/
 		...
 		function esf_reset_panel(element_id) {
 			...
 			$the_tree.find('input:checkbox').attr('checked', false);
 		...
 		$panel_content.find('input:checkbox').on('change', function() {
 			if ( $(this).is(':checked') ) {
 				$(this).closest('li').addClass('esf-item-checked').find('ul').find('li').addClass('esf-item-checked').find('input:checkbox').attr('checked', true);
 				esf_count_selected($(this));
 			}
 			else {
 				$(this).closest('li').removeClass('esf-item-checked').find('ul').find('li').removeClass('esf-item-checked').find('input:checkbox').attr('checked', false);
 				esf_count_selected($(this));
 			}
 			// AJAX update list
 		...
 		$panel_content.on('change', '.esf-checkbox-select-all', function() {
			if ( $(this).is(':checked') ) {
 				$(this).closest('li').nextAll('li').addClass('esf-item-checked').find('input:checkbox').attr('checked', true);
 				$(this).closest('li').nextAll('li').find('li').addClass('esf-item-checked');
			}
			else {
				$(this).closest('li').nextAll('li').removeClass('esf-item-checked').find('input:checkbox').attr('checked', false);
